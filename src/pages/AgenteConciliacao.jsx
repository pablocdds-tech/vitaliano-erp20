...
import { Send, Landmark, Loader2, Trash2, Paperclip, X, CheckCircle2, AlertTriangle } from 'lucide-react';
import ReactMarkdown from 'react-markdown';
import { cn } from '@/lib/utils';
import { toast } from 'sonner';

const SUGGESTED = [
  { icon: '📂', text: 'Envie um arquivo OFX ou extrato bancário para conciliar' },
  { icon: '🔍', text: 'Quais lançamentos estão pendentes de conciliação?' },
  { icon: '📊', text: 'Mostrar resumo de conciliação deste mês' },
  { icon: '⚠️', text: 'Verificar possíveis lançamentos duplicados' },
];
            ...
            {isUser ? (
              ...
              <ReactMarkdown
                ...
                components={{
                  ...
                  blockquote: ({ children }) => (
                    <blockquote className="border-l-4 border-blue-300 pl-3 my-2 text-slate-600 italic">{children}</blockquote>
                  ),
                  table: ({ children }) => (
  ...
  const initConversation = async () => {
    try {
      const conv = await base44.agents.createConversation({
        agent_name: 'agente_conciliacao',
        metadata: { name: 'Sessão Conciliação' },
      });
  ...
  const sendMessage = async (text) => {
    ...
    try {
      await base44.agents.addMessage(conversation, {
        role: 'user',
        content: msg || 'Analise este extrato bancário e faça a conciliação.',
    ...
    <div className="flex flex-col h-[calc(100vh-5rem)] max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-slate-200">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-slate-900">Agente de Conciliação</h1>
            <p className="text-xs text-slate-500">Matching automático de extratos bancários</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <div className="hidden sm:flex items-center gap-1.5 text-xs text-amber-600 bg-amber-50 border border-amber-200 px-3 py-1.5 rounded-lg">
            ...
            Atualiza registros somente após confirmação
          </div>
        ...
        {initializing ? (
          ...
          <div className="flex flex-col items-center justify-center h-full gap-6 text-center px-4">
            <div>
              ...
              <h2 className="text-lg font-semibold text-slate-800">Agente de Conciliação Bancária</h2>
              <p className="text-sm text-slate-500 mt-1 max-w-md">
                Envie um extrato OFX ou cole suas transações bancárias. Vou cruzar com os lançamentos do sistema, detectar duplicidades, sugerir rateios e classificar automaticamente — mas só aplico após sua confirmação.
              </p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 w-full max-w-xl">
              {SUGGESTED.map((s) => (
                <button
                  ...
                  className="text-left text-sm px-4 py-3 rounded-xl border border-slate-200 bg-white hover:border-blue-300 hover:bg-blue-50 transition-colors text-slate-700 flex items-center gap-2"
                >
      ...
      {attachedFiles.length > 0 && (
        <div className="flex flex-wrap gap-2 pb-2">
          {attachedFiles.map((f, i) => (
            <div key={i} className="flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-lg px-3 py-1.5 text-xs text-blue-700">
      ...
      <div className="pt-4 border-t border-slate-200 space-y-2">
        <div className="flex gap-2">
          <input ref={fileInputRef} type="file" accept=".ofx,.csv,.txt,image/*,.pdf" multiple className="hidden" onChange={handleFileSelect} />
          <Button
            variant="outline"
            size="icon"
            ...
            title="Anexar extrato (OFX, CSV, PDF)"
          >
          ...
          <Input
            ...
            placeholder={attachedFiles.length > 0 ? 'Adicione uma mensagem ou envie direto...' : 'Cole transações ou envie um extrato OFX...'}
        ...
        <div className="flex items-center justify-center gap-4 text-xs text-slate-400">
          <span className="flex items-center gap-1"><CheckCircle2 className="h-3 w-3 text-blue-500" /> Matching automático com CP/CR</span>
          <span className="flex items-center gap-1"><AlertTriangle className="h-3 w-3 text-amber-500" /> Aplica somente após confirmação</span>
        </div>
      </div>
    </div>
  );
