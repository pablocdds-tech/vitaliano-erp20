...
Seja preciso e sempre peça confirmação antes de sugerir alterações. Responda sempre em português.`;

const SUGGESTED = [
  { icon: '📂', text: 'Envie um arquivo OFX ou extrato bancário para conciliar' },
  { icon: '🔍', text: 'Como funciona a conciliação bancária?' },
  { icon: '📊', text: 'Quais são os principais erros de conciliação?' },
...
export default function AgenteConciliacao() {
  ...
  const sendMessage = async (text) => {
    ...
    const fileInfo = attachedFiles.length > 0
      ? `\n\n[Arquivo(s) anexado(s): ${attachedFiles.map(f => f.name).join(', ')}]`
      : '';

    const fullMsg = (msg || 'Analise este extrato bancário.') + fileInfo;
    ...
    <div className="flex flex-col h-[calc(100vh-5rem)] max-w-4xl mx-auto">
        ...
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-6 text-center px-4">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1 max-w-md">Envie extratos OFX, CSV ou faça perguntas sobre conciliação bancária.</p>
            </div>
      ...
      <div className="pt-4 border-t border-slate-200 space-y-2">
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="icon" onClick={() => fileInputRef.current?.click()} disabled={uploading || loading} className="shrink-0 rounded-xl border-dashed" title="Anexar extrato">
            {uploading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Paperclip className="h-4 w-4" />}
          </Button>
          <Input ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown}
            placeholder={attachedFiles.length > 0 ? 'Adicione uma mensagem...' : 'Cole transações ou faça perguntas...'} disabled={loading} className="flex-1 rounded-xl" />
          <Button onClick={() => sendMessage()} disabled={(!input.trim() && attachedFiles.length === 0) || loading} className="bg-blue-600 hover:bg-blue-700 rounded-xl px-4">
